...
<p class="woocommerce-result-count" role="alert" aria-relevant="all" <?php echo ( empty( $orderedby ) || 1 === intval( $total ) ) ? '' : 'data-is-sorted-by="true"'; ?>>
	...
	if ( 1 === intval( $total ) ) {
		...
		printf( _n( 'Showing all %1$d Product', 'Showing all %1$d Products', $total, 'kapee' ) . $orderedby_placeholder, $total, esc_html( $orderedby ) );
	} else {
		...
		printf(
			_nx(
				'%1$d–%2$d Product of %3$d Product',
				'%1$d–%2$d Products of %3$d Products',
				$total,
