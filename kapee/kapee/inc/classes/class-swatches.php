	...
	class Kapee_Woocommerce{
		...
		*/
		function kapee_save_attribute_swatch_size($attribute_id,$attribute) {
			...
			$swatch_display_size 	= isset($_POST[$prefix.'swatch_display_size']) ? sanitize_text_field( $_POST[$prefix.'swatch_display_size'] ) : 'normal';
			$swatch_display_style 	= isset($_POST[$prefix.'swatch_display_style']) ? sanitize_text_field( $_POST[$prefix.'swatch_display_style'] ) : 'square';
		...
		*/
		function kapee_save_attr_extra_fields($term_id) {
			...
			$kapee_attachment_id = !empty($_POST[$prefix.'kapee_attachment_id']) ? sanitize_text_field( $_POST[$prefix.'kapee_attachment_id'] ) : '';
		...
		*/		
		...
		function save_category_fields( $term_id ) {
			...
			$kapee_attachment_id = !empty($_POST[$prefix.'kapee_attachment_id']) ? sanitize_text_field( $_POST[$prefix.'kapee_attachment_id'] ) : '';
			$sidebar = !empty($_POST[$prefix.'sidebar']) ? sanitize_text_field( $_POST[$prefix.'sidebar'] ) : '';
			$sidebar_title_color = !empty($_POST[$prefix.'sidebar_title_color']) ? sanitize_text_field( $_POST[$prefix.'sidebar_title_color'] ) : '';
