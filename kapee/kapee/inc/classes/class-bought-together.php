...
if (!class_exists('Kapee_Bought_Together')) {
	...
	class Kapee_Bought_Together{
		...
		public function kapee_bought_save_data($product_id) {
			...
			if ( isset( $_POST[$this->prefix.'product_ids'] ) && is_array( $_POST[$this->prefix.'product_ids'] ) ) {
				$data = array_map( 'sanitize_text_field', $_POST[$this->prefix.'product_ids'] );
			} else {
				$data = [];
			}
		...
		public function kapee_is_enable_FBT(){
