...
class Kapee_Cookie_Notice {
	...
	public function kapee_cookie_setted() {
		...
		if(isset( $_COOKIE[self::$cookie['name']] ) && $_COOKIE[self::$cookie['name']] === 'true' ){
			$ookie_set = true;
		}
		return $ookie_set;
	}
	...
	public function kapee_wp_print_footer_scripts() {
		...
		if (!kapee_get_option('cookie-notice', 0)) {
			return false;
		}
		if (!kapee_get_option('cookie-refuse-opt', 0)) {
			return false;
		}
		$scripts = trim(wp_kses_post(kapee_get_option('cookie-refuse-code', '')));
		
		if ( $this->kapee_cookie_setted() && ! empty( $scripts ) ) {
			?>
			<script type='text/javascript'>
				<?php echo ' '.$scripts ; // phpcs:disable WordPress.XSS.EscapeOutput.OutputNotEscaped ?>
			</script>
			<?php
		}
	}
