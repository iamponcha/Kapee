...
class Kapee_Admin {
	...
	public function save_custom_fields($menu_id, $menu_item_db_id, $args){
		...
		foreach ( $custom_fields as $key ) {		
			$raw_value  = isset($_POST['menu-item-'.$key][$menu_item_db_id]) ? wp_unslash( $_POST['menu-item-'.$key][$menu_item_db_id] ) : '';
			$value = $this->sanitize_menu_field_value( $key, $raw_value );
			update_post_meta( $menu_item_db_id, '_menu_item_kapee_'.$key, $value );
		}
	}
	
	protected function sanitize_menu_field_value( $key, $value ) {
