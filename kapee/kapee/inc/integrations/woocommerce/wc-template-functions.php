...
if( ! function_exists( 'kapee_ajax_wishlist_count' ) ) :
	function kapee_ajax_wishlist_count() {
		...
		wp_send_json( kapee_wishlist_count() );
		die();
	}
endif;

if( ! function_exists( 'kapee_get_compare_count_2x_ajax_safe' ) ) :
	function kapee_get_compare_count_2x_ajax_safe() {
	...
	function kapee_ajax_compare_count(){
		...
		if( defined( 'YITH_WOOCOMPARE' ) ){			
			if(class_exists('YITH_WooCompare_Products_List') ){
				...
				$product_count = $PL_YITH_Products_List->count(); 
				
			}else{			
				/*Support 2.X */
				$product_count = kapee_get_compare_count_2x_ajax_safe();
			}			
		}
		wp_send_json_success( $product_count );
	}
}
...
if ( ! function_exists( 'kapee_product_loop_compare_button' ) ) :
	...
	function kapee_product_loop_compare_button() {
		...
		if ( version_compare( YITH_WOOCOMPARE_VERSION, '3.0.0', '>=' ) ) {
			$show_button_in = get_option( 'yith_woocompare_show_compare_button_in', 'both' );
		...
		<div class="compare-button">
			<?php printf( '<a href="%s" class="%s" data-product_id="%d" rel="nofollow">%s</a>',
						kapee_compare_add_product_url( $product_id ),
						'compare'.' '.$compare_button_style.$added,
						$product_id,
						$button_text ); 
			?>
		</div>
...
if ( ! function_exists( 'kapee_single_product_degree360_btn' ) ) :
	...
	function kapee_single_product_degree360_btn(){
		...
		if( ! kapee_get_option( 'product-360-degree', 1 ) || ! $post ) {
			return;
		}
		...
		if( ! empty( $gallery_images ) ){ ?>
			<div class="product-360-degree-btn">
				<a href="#kapee-360-degree-popup" ><?php esc_html_e('360 Degree', 'kapee'); ?></a>
			</div>			
		<?php }
	}
endif;

if ( ! function_exists( 'kapee_single_product_360_degree_popup' ) ) :
	/**
	 * Single Product 360 Degree Popup
	 */
	function kapee_single_product_360_degree_popup(){
		...
		<div id="kapee-360-degree-popup" class="kapee-360-degree-popup mfp-hide">
			<div class="kapee-360-degree-content">
		...
		wp_add_inline_script('threesixty',
			'jQuery(document).ready(function( $ ) {
				$(".kapee-360-degree-content").ThreeSixty({
...
if ( ! function_exists( 'kapee_single_product_stock_availability' ) ) :
	...
	function kapee_single_product_stock_availability() {
		...
		if( ! kapee_get_option( 'single-product-availability', 1 ) ) return;
