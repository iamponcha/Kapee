...
if( ! function_exists( 'kapee_woocommerce_setup' ) ) :
	...
	add_action( 'init', 'kapee_woocommerce_setup' , 15 );
endif;
...
if( ! function_exists( 'kapee_manage_woocommerce_hooks' ) ) {
	...
	function kapee_manage_woocommerce_hooks() {
		...
		if( isset( $GLOBALS['WC_Brands'] ) ){				
			if( ! kapee_get_option( 'single-product-brands', 0 ) ) {	
...
function kapee_wishlist_count(){
	$count = 0;
	/* Support 4.0.0*/
	if( class_exists( 'YITH_WCWL_Wishlists' ) ){
		return YITH_WCWL_Wishlists()->count_items_in_wishlist();
	}
	/*Support 3.0.0 */
...
if( ! function_exists( 'kapee_product_quick_view' ) ){
	...
	function kapee_product_quick_view(){
		...
		if( isset( $_REQUEST['pid'] ) ) {
			$pid = sanitize_text_field( (int) wp_unslash( $_REQUEST['pid'] ) );
		}
...
if ( !function_exists('kapee_ajax_get_block') ) {
	function kapee_ajax_get_block(){
		...
		$title = isset($_POST['title']) ? (int) sanitize_text_field( $_POST['title'] ) : '';
		if( $post_id ){
			...
			echo do_shortcode('[kapee_block id="'.esc_attr( $post_id ).'"]');
			echo '</div>';
		}else{
...
if ( ! function_exists( 'kapee_all_add_to_cart' ) ) {
	...
	function kapee_all_add_to_cart() {
		...
		$variation_id      = empty( $_POST['variation_id'] ) ? 0 : sanitize_text_field( wp_unslash ( $_POST['variation_id'] ) );
		$variation         = empty( $_POST['variation'] ) ? array() : array_map( 'sanitize_text_field', wp_unslash( $_POST['variation'] ) );
...
if ( ! function_exists( 'kapee_related_products_args' ) ){
	...
	function kapee_related_products_args($args){		
		$args['posts_per_page'] = ( int ) kapee_get_option( 'related-upsells-products', 12 );
		return $args;
	}
}
...
if( ! function_exists( 'kapee_get_products_tab_data' ) ){
    ...
    function kapee_get_products_tab_data(){
        ...
        $datasource 	= isset( $_POST['datasource'] ) ? sanitize_text_field( wp_unslash( $_POST['datasource'] ) ) : '';
